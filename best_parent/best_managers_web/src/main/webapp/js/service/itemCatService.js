...
app.service('itemCatService',function($http){
	...
	this.search=function(page,rows,searchEntity){
		console.log(searchEntity.parentId);
		return $http.post('../itemCat/search.do?page='+page+"&rows="+rows, searchEntity);
	};


	this.findByParentId=function (pageNum,size,parentId) {

		return $http.get('../itemCat/findCatByParentId.do?parentId='+parentId+"&rows="+size+"&pageNum="+pageNum);
    }
});
